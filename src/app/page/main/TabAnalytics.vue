...
export default {
  ...
  computed: {
    ...
    listsInView() {
      const allTabsUrl = []
      const allTabsTitle = []
      this.listsToDisplay.forEach(l => {
        l.tabs.forEach(t => {
          allTabsUrl.push(t.url)
          allTabsTitle.push(t.title)
        })
      })

      const mostOpenedUrl = {}
      allTabsUrl.forEach(element => {
        mostOpenedUrl[element] = (mostOpenedUrl[element] || 0) + 1
      })
      const mostOpenedUrlOrdered = Object.entries(mostOpenedUrl).sort((a, b) => b[1]-a[1])

      const mostOpenedTitle = {}
      allTabsTitle.forEach(element => {
        mostOpenedTitle[element] = (mostOpenedTitle[element] || 0) + 1
      })
      const mostOpenedTitleOrdered = Object.entries(mostOpenedTitle).sort((a, b) => b[1]-a[1])

      console.log({
        allTabsUrl,
        allTabsUrlCount: allTabsUrl.length,
        mostOpenedUrlOrdered,
        mostOpenedTitleOrdered,
        currentPage: this.currentPage,
        listsToDisplay: this.listsToDisplay,
        inPageLists: this.inPageLists(this.currentPage, this.listsToDisplay)
      })
      return this.inPageLists(this.currentPage, this.listsToDisplay).slice(0, 10)
    },
    pageLength() {
